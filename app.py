...
def extract_claim(text):
    words = [w for w in text.lower().split()[:70] if len(w) > 4][:10]
...
article = st.text_area("Paste article here:", height=300)
...
if st.button("Check Truth", type="primary", use_container_width=True):
    if not article.strip():
        ...
        c1.metric("ML", "Real" if pred==1 else "Fake", f"{prob:.0f}%")
        c2.metric("Trusted News", news["credible"])
        c3.metric("Gemini", gemini["verdict"])
        if gemini["reason"]: st.info(gemini["reason"])
        if news["results"]:
            st.success("Trusted sources:")
            for r in news["results"]:
                st.markdown(f"• [{r.get('title','')[:100]}]({r.get('link','#')})")

st.caption("Working 100% on Streamlit Cloud – no spaCy, no subprocess")
